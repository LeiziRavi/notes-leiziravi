...
# Use these funstions to convert any string to appropriate Python data type
...
with open('sample.csv', encoding='utf-8', newline='') as f:
    ...
    for i, row in reader:
        ...
        # id and appropriate data types, add to people dictionary````````````.
        newdict = dict({
            'first_name': fname(row[0]), 'last_name': lname(row[0]), 'birth_year': integer(row[1]), 'date_joined': date(row[2]),
